...
| **B2** | a document id `sidebars.json` names that equals no id the tree DECLARES, or an `{"type": "autogenerated"}` `dirName` naming no directory. A file's id is its path without the extension, with a frontmatter `id` replacing the last segment |
...
a sidebar `"type"` this gate does not enumerate; a symlink cycle; and **orphans**, meaning files no
sidebar entry references and no `autogenerated` entry covers. The four link classes are excluded
...
line by line, so link-shaped text between them can still be extracted. And rule 1 URL-decodes every
target, so a file whose NAME literally contains a percent escape (`guide%20name.md`) linked as
`./guide%20name.md` decodes to `guide name.md` and reds.
