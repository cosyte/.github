...
| [`release.yml`](.github/workflows/release.yml) | Changesets → npm publish **with provenance** → docs artifacts → GitHub release → `repository_dispatch` to `cosyte/docs`. On failure, uploads the redacted npm debug log as a run artifact | every published parser |
