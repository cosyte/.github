...
// proves the workflow that RUNS those scripts still covers them, because `scripts` is now a required
// status check and a required context is only worth what it actually executes.
//
// ▶ THE TWO WAYS A REQUIRED CONTEXT STOPS GATING WITHOUT ANYONE EDITING A RULESET, and neither is
...
//   and a new conditional, and it is NOT enough to catch an author who restructures the file into a
//   shape these anchors no longer locate. The `the anchors located something` assertions exist so
//   that case reds rather than passing vacuously, which is the failure mode of every text-anchored
...
/**
 * A glob of the shape the workflow uses, as a RegExp. `*` matches within one path segment only,
 * which is exactly why a file added under `test/unit/` is NOT selected by `test/*.test.mjs`.
 */
...
test('the `scripts` job runs node:test over a glob, quoted so node does the expansion', () => {
  const run = jobBody('scripts').match(/^ {8}run: node --test "([^"]+)"$/m);
  assert.ok(run, 'the `node --test` step was not located in the `scripts` job');
  assert.equal(run[1], 'test/*.test.mjs');
});
